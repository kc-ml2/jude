...
class TwoHeadsNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.blur_kernel_size == 37:
            k = self.kernels_end(k5)
            diffY = torch.tensor([self.blur_kernel_size - k.size()[2]])
            diffX = torch.tensor([self.blur_kernel_size - k.size()[3]])

            k = F.pad(
                k, [diffX // 2, diffX - diffX // 2, diffY // 2, diffY - diffY // 2]
            )
        else:
            k = self.kernels_end(k5)
...
class TwoHeadsNetwork_v2(nn.Module):
    def __init__(self, K=9, blur_kernel_size=33, scale=1, bilinear=False, no_softmax=False):
        super(TwoHeadsNetwork_v2, self).__init__()
        ...
        self.down1 = Down(int(16*self.scale), int(16*self.scale))
        self.down2 = Down(int(16*self.scale), int(32*self.scale))
        self.down3 = Down(int(32*self.scale), int(64*self.scale))
        self.down4 = Down(int(64*self.scale), int(128*self.scale))
        self.down5 = Down(int(128*self.scale), int(256*self.scale))
        self.feat = nn.Sequential(
            nn.Conv2d(int(256*self.scale), int(256*self.scale), kernel_size=3, padding=1),
            nn.LeakyReLU(inplace=True),
        )

        self.up1 = Up(int(256*self.scale),int(256*self.scale), int(128*self.scale), bilinear)
        self.up2 = Up(int(128*self.scale),int(128*self.scale), int(64*self.scale), bilinear)
        self.up3 = Up(int(64*self.scale),int(64*self.scale), int(32*self.scale), bilinear)
        self.up4 = Up(int(32*self.scale),int(32*self.scale), int(16*self.scale), bilinear)
        self.up5 = Up(int(16*self.scale),int(16*self.scale), int(16*self.scale), bilinear)

        self.masks_end = nn.Sequential(
            nn.Conv2d(int(16*self.scale), int(16*self.scale), kernel_size=3, padding=1),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(int(16*self.scale), int(16*self.scale), kernel_size=3, padding=1),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(int(16*self.scale), K, kernel_size=3, padding=1),
            nn.Softmax(dim=1),
        )
        ...
        self.kernel_up1 = Up(int(256*self.scale), int(256*self.scale), int(128*self.scale), bilinear)
        self.kernel_up2 = Up(int(128*self.scale), int(128*self.scale), int(64*self.scale), bilinear)
        self.kernel_up3 = Up(int(64*self.scale), int(64*self.scale), int(64*self.scale), bilinear)
        self.kernel_up4 = Up(int(64*self.scale), int(32*self.scale), int(32*self.scale), bilinear)
        self.kernel_up5 = Up(int(32*self.scale), int(16*self.scale), int(16*self.scale), bilinear)

        self.kernels_end = nn.Sequential(
            nn.Conv2d(int(16*self.scale), int(16*self.scale), kernel_size=2, padding=1),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(int(16*self.scale), int(16*self.scale), kernel_size=3, padding=1),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(int(16*self.scale), K, kernel_size=3, padding=1),
...
class KernelsNetwork(nn.Module):
    def __init__(self, K=1, blur_kernel_size=37, bilinear=False, no_softmax=False):
        super(KernelsNetwork, self).__init__()
...
if __name__ =="__main__":
    ...
    model = TwoHeadsNetwork_v2(scale=2)
    print(
