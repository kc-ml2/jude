...
from basicsr.data.transforms import augment, paired_random_crop, paired_crop
...
class LOLImageDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.generate_lol_img = opt.get('generate_lol_img', False)
        self.add_gaussian_noise = opt.get('add_noise', False)
        ...
        self.kernel_path = opt['dataroot_kernel']
        ...
        self.gt_folder, self.lq_folder, self.lq_folder_noise = opt.get('dataroot_gt', None), opt.get('dataroot_lq', None), opt.get('dataroot_lq_noise', None)
        if self.generate_lol_img:
            ...
            self.paths = paired_paths_from_folder([self.lq_folder, self.gt_folder], ['lq', 'gt'], self.filename_tmpl, self.opt)
            # self.paths += paired_paths_from_folder([self.lq_folder_noise, self.gt_folder], ['lq', 'gt'], self.filename_tmpl, self.opt)
    ...
    def __getitem__(self, index):
        ...
        # get kernel
        if 'our' in self.opt['dataset']:
            kernel_name = osp.basename(osp.dirname(osp.dirname(lq_path)))
            kernel_path = osp.join(self.kernel_path, kernel_name) + ".png"
            ...
            if self.opt['phase'] == 'train':
                ...
                # flip, rotation
                img_gt, img_lq, kernel = augment([img_gt, img_lq, kernel], self.use_flip, self.use_rot)

            # BGR to RGB, HWC to CHW, numpy to tensor
            img_gt, img_lq, kernel = img2tensor([img_gt, img_lq, kernel], bgr2rgb=True, float32=True)
            # normalize
            # normalize(img_lq, self.mean, self.std, inplace=True)
            # normalize(img_gt, self.mean, self.std, inplace=True)
        else:
            kernel = np.zeros((32, 32, 1))
            # augmentation for training
            if self.opt['phase'] == 'train':
                # random crop
                if self.generate_lol_img:
                ...
                img_gt, img_lq = paired_crop(img_gt, img_lq)
            # BGR to RGB, HWC to CHW, numpy to tensor
            img_gt, img_lq, kernel = img2tensor([img_gt, img_lq, kernel], bgr2rgb=True, float32=True)
            # normalize
            # normalize(img_lq, self.mean, self.std, inplace=True)
        ...
        return {'lq': img_lq, 'gt': img_gt, 'kn': kernel, 'nm': lq_path}
