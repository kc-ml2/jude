...
class TwoHeadsNetwork_v2(nn.Module):
    def __init__(
        self, K=1, blur_kernel_size=33, scale=1, bilinear=False, no_softmax=False
    ):
        ...
        self.inc_rgb = nn.Sequential(
            nn.Conv2d(3, 16 * scale, kernel_size=3, padding=1),
        ...
        self.feat = nn.Sequential(
            nn.Conv2d(
                int(256 * self.scale), int(256 * self.scale), kernel_size=3, padding=1
            ),
            nn.LeakyReLU(inplace=True),
        )
        ...
        self.up2 = Up(
            int(128 * self.scale), int(128 * self.scale), int(64 * self.scale), bilinear
        )
        self.up3 = Up(
            int(64 * self.scale), int(64 * self.scale), int(32 * self.scale), bilinear
        )
        self.up4 = Up(
            int(32 * self.scale), int(32 * self.scale), int(16 * self.scale), bilinear
        )
        self.up5 = Up(
            int(16 * self.scale), int(16 * self.scale), int(16 * self.scale), bilinear
        )

        self.masks_end = nn.Sequential(
            nn.Conv2d(
                int(16 * self.scale), int(16 * self.scale), kernel_size=3, padding=1
            ),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(
                int(16 * self.scale), int(16 * self.scale), kernel_size=3, padding=1
        ...
        self.kernel_up3 = Up(
            int(64 * self.scale), int(64 * self.scale), int(64 * self.scale), bilinear
        )
        self.kernel_up4 = Up(
            int(64 * self.scale), int(32 * self.scale), int(32 * self.scale), bilinear
        )
        self.kernel_up5 = Up(
            int(32 * self.scale), int(16 * self.scale), int(16 * self.scale), bilinear
        )

        self.kernels_end = nn.Sequential(
            nn.Conv2d(
                int(16 * self.scale), int(16 * self.scale), kernel_size=2, padding=1
            ),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(
                int(16 * self.scale), int(16 * self.scale), kernel_size=3, padding=1
