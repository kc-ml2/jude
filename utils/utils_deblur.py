...
def pad(h, shape_x):
    ...
    i1, j1 = (
        np.int32((shape_x[0] - shape_h[0]) / 2) + offset,
        np.int32((shape_x[1] - shape_h[1]) / 2) + offset,
    )
...
def crop(h, shape_crop):
    shape_h = np.shape(h)
    i1, j1 = np.int32((shape_h[0] - shape_crop[0]) / 2), np.int32(
        (shape_h[1] - shape_crop[1]) / 2
    )
    i2, j2 = np.int32((shape_h[0] + shape_crop[0]) / 2), np.int32(
        (shape_h[1] + shape_crop[1]) / 2
    )
...
def isotropic_total_variation(I, device):
    ...
    Returns:
        torch.Tensor [H, W] per-pixel total variation
    """

    gauss_1d = torch.tensor([1, 2, 1], dtype=torch.float32).to(device)
    f_tv = gauss_1d[:, None] @ gauss_1d[None, :]
    f_tv = f_tv / torch.sum(f_tv)

    # I = torch.unsqueeze(I, 0)  # Adding batch dimension
    f_tv = torch.unsqueeze(f_tv, 0)  # Adding batch dimension

    I_blur = F.conv2d(I, f_tv.view(1, 1, 3, 3), padding=1)
    I_sq_blur = F.conv2d(I**2, f_tv.view(1, 1, 3, 3), padding=1)
    isotropic_tv = torch.abs(I_sq_blur - I_blur**2)

    return isotropic_tv.squeeze(0)
