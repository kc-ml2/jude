...
if __name__ == "__main__":
    ...
    if "clipiqa+" in args.metrics:  # 224 higher value better image
        iqa_ssim = pyiqa.create_metric("clipiqa+_rn50_512").to(device)
        iqa_ssim.eval()
    if "musiq-koniq" in args.metrics:
        # iqa_lpips = pyiqa.create_metric('lpips').to(device)
        iqa_lpips = pyiqa.create_metric("musiq-spaq").to(device)
        iqa_lpips.eval()
    if "maniqa" in args.metrics:  # 224 higher value better image
        iqa_nrqm = pyiqa.create_metric("maniqa-kadid").to(device)
        iqa_nrqm.eval()
    ...
    if "topiq_nr" in args.metrics:
        iqa_brisque = pyiqa.create_metric("topiq_nr-spaq").to(device)
        iqa_brisque.eval()
    ...
    if "liqe" in args.metrics:  # 384 -> higher the better
        iqa_musiq = pyiqa.create_metric("liqe_mix", as_loss=False).to(device)
        iqa_musiq.eval()
    ...
    for i, img_out_path in enumerate(img_out_paths):
        ...
        with torch.no_grad():
            ...
            if iqa_psnr is not None:
                ...
                psnr = iqa_psnr(img_out_rgb).item()
            ...
            if iqa_ilniqe is not None:
                ...
                #                                  std=(0.229, 0.224, 0.225))])
                img_out_ = transform(img_out)
                ilniqe = iqa_ilniqe(img_out_).item()
    ...
    result_file = open(os.path.join(args.result_path, "test_result_v2.txt"), "w")
